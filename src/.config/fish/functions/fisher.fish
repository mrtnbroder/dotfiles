...
function fisher -a cmd -d "fish package manager"
    ...
    set -q fisher_path
    or set -g fisher_path $fish_config
    ...
    if test -e "$fisher_path/fishfile"
        and test ! -e "$fishfile"
...
function _fisher_fetch
    ...
    set -l next_pkgs
    set -l local_pkgs
    set -q fisher_user_api_token
    and set -l curl_opts -u $fisher_user_api_token
...
function _fisher_rm -a pkg
    for src in $pkg/{conf.d,completions,functions}/**.* $pkg/*.fish
        ...
        switch $src
            case $pkg/conf.d\*
                test "$filename.fish" = "$target"
                and emit "$filename"_uninstall
                set target conf.d/$target
            case $pkg/completions\*
                test "$filename.fish" = "$target"
                and complete -ec $filename
                set target completions/$target
            case $pkg/{,functions}\*
                test "$filename.fish" = "$target"
                and functions -e $filename
                switch $target
                    case uninstall.fish
                        source $src
                        continue
